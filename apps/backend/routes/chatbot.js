...
const PRODUCT_KEYWORDS = [
  { names: ["cà phê", "cafe", "coffee"], normalized: "cà phê" },
  { names: ["tiêu", "hồ tiêu", "pepper"], normalized: "tiêu" },
  { names: ["lúa", "gạo", "rice"], normalized: "lúa" },
  { names: ["cao su", "rubber"], normalized: "cao su" },
  { names: ["ca cao", "cacao"], normalized: "ca cao" },
];
...
const REGION_KEYWORDS = [
  ...
  "sông cửu long", "song cuu long", "mekong delta"
];

const PRICE_KEYWORDS = ["giá", "bao nhiêu", "hiện tại", "price", "cost"];
const HISTORY_KEYWORDS = ["lịch sử", "trước đây", "các ngày", "biến động", "history"];
const TREND_KEYWORDS = ["xu hướng", "đang tăng", "đang giảm", "trend"];
const PREDICT_KEYWORDS = ["dự đoán", "sắp tới", "vài ngày", "tương lai", "predict"];
const COMPARE_KEYWORDS = ["so sánh", "compare", "khác nhau"];
const ALERT_KEYWORDS = ["cảnh báo", "alert", "thông báo", "đặt giá"];
const HELP_KEYWORDS = ["hướng dẫn", "giúp", "help"];
...
function analyzeTrend(prices) {
  ...
  const dataSize = Math.min(prices.length, 7);
...
function calculateVolatility(prices) {
  ...
  for (let i = 1; i < prices.length; i++) {
    changes.push(Math.abs((prices[i - 1] - prices[i]) / prices[i] * 100));
  }
...
function predictTrend(prices, dataPoints) {
  ...
  const longTermAvg = longTerm.reduce((a, b) => a + b, 0) / longTerm.length;
  const shortTermAvg = shortTerm.reduce((a, b) => a + b, 0) / shortTerm.length;
  ...
  if (momentum > 1) {
    trendDirection = "tăng";
    trendDescription = `Xu hướng TĂNG ${trendStrength} (${momentum.toFixed(2)}%)`;
  } else if (momentum < -1) {
    trendDirection = "giảm";
    trendDescription = `Xu hướng GIẢM ${trendStrength} (${momentum.toFixed(2)}%)`;
  } else {
  ...
  if (!hasLongHistory) {
    confidence = "low";
    reason = `Dữ liệu còn ít (${dataPoints} ngày), độ chính xác hạn chế. `;
  } else {
    reason = `Dựa trên ${dataPoints} ngày dữ liệu. `;
  }
  ...
  try {
    ...
    if (analysis.intent === INTENTS.GET_PRICE) {
      ...
      if (rows.length === 0) {
        return res.json({
          type: "INFO",
          text: `Không tìm thấy giá ${analysis.product}${analysis.region ? ` tại ${analysis.region}` : ''}. Thử tìm kiếm sản phẩm khác? 🔍`,
        });
      }
      ...
      return res.json({
        ...
        message: rows.length > 1 ? `Tìm thấy ${rows.length} kết quả:` : null
      });
    }
    ...
    if (analysis.intent === INTENTS.GET_HISTORY) {
      ...
      if (!product) {
        return res.json({
          type: "INFO",
          text: `Không tìm thấy sản phẩm ${analysis.product}${analysis.region ? ` tại ${analysis.region}` : ''}.`,
        });
      }
    ...
    if (analysis.intent === INTENTS.GET_TREND) {
      ...
      return res.json({
        type: "TREND",
        text: `Xu hướng giá ${product.name} ${product.region ? `(${product.region})` : ''} ${trendMap[trendAnalysis.trend]}`,
        data: {
          product: product.name,
          region: product.region,
    ...
    if (analysis.intent === INTENTS.PREDICT_TREND) {
      ...
      console.log(`📊 Sản phẩm: ${product.name}, Số ngày dữ liệu: ${history.length}`);

      if (history.length < 2) {
        return res.json({
          type: "INFO",
          text: `Chưa đủ dữ liệu để dự đoán xu hướng cho ${product.name}. Cần ít nhất 2 ngày dữ liệu (hiện có ${history.length} ngày).`,
        });
      }
      ...
      if (history.length >= 3) {
        ...
        responseText += `\n💰 Giá hôm nay: ${Number(prediction.currentPrice).toLocaleString()} VNĐ`;
        responseText += `\n📅 Dữ liệu: ${prediction.dataPoints} ngày`;
      }
    ...
    if (analysis.intent === INTENTS.SET_ALERT) {
      ...
      return res.json({
        type: "SUCCESS",
        text: `✅ Đã đặt cảnh báo giá ${analysis.product} ${analysis.condition === 'above' ? 'trên' : 'dưới'} ${analysis.threshold.toLocaleString()} VNĐ`,
      });
    }
    ...
    if (analysis.intent === INTENTS.GET_ALERT) {
      ...
      alerts.forEach((a, index) => {
        text += `${index + 1}. ${a.product_name} ${a.alert_condition === "above" ? "trên" : "dưới"
          } ${Number(a.target_price).toLocaleString()} VNĐ\n`;
      });
