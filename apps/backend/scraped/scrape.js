...
const DURIAN_URL = "https://giacaphe.com/gia-sau-rieng-hom-nay/";
...
async function scrapeDurian(page, existing) {
    ...
    try {
        ...
        const rows = await page.$$eval("table.price-table tbody tr", trs =>
            trs.map(tr => {
                const tds = tr.querySelectorAll("td");
                const Loai = tds[0]?.innerText.trim() || "";
                const ThapNhat = tds[1]?.innerText.trim() || "";
                const CaoNhat = tds[2]?.innerText.trim() || "";

                const priceValue = parseInt(CaoNhat.replace(/\D/g, "")) || 0;

                return { Loai, ThapNhat, CaoNhat, priceValue };
            })
        );
        ...
        // -------------------------
        for (const r of rows) {
            const regionName = `${r.Loai}`;
            const regionKey = `${r.Loai}`;
