...
export default function AdminSettings() {
  ...
  // Hàm chạy Scraper thủ công
  const handleRunScraper = async () => {
    setScraperRunning(true)
    try {
      // Gọi API trigger scraper (Backend cần có route này, hoặc giả lập)
      // await api.post("/admin/scrape-trigger") 
      
      // Giả lập chạy mất 3s
      await new Promise(r => setTimeout(r, 3000))
      
      toast({ title: "Thành công", description: "Hệ thống đang cào dữ liệu mới..." })
    } catch (error) {
      toast({ title: "Lỗi", description: "Không thể kích hoạt Scraper", variant: "destructive" })
    } finally {
